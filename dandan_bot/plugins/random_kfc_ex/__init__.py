...
kfc = [
    ...
    "肯德基给你们多少钱了，让你们周周给编段子，疯狂星期四真的那么有诱惑力？我偏不信，v我50，我去试试",
    "处cp吗？以后只宠你一人～“爱情??不是??随便许诺??好了?不想?再说?了? 没错 是我那么多的冷漠 让你感觉到无比的寂寞很? 不过 一个女人的?? 不仅仅渴望得到的一个承诺? 我害怕欺骗?也害怕寂寞? 更害怕你不请我吃疯狂星期四?",
...
crazy = on_regex(pattern=r"^疯狂星期\S\S*", priority=15, rule=to_me())
crazy_jp = on_regex(pattern=r"^狂乱\S曜日\S*", priority=15, rule=to_me())
crazy_en = on_regex(pattern=r"^[C|c][R|r][A|a][Z|z][Y|y]\s*\S*", priority=15, rule=to_me())
...
def rndKfc(day: str):
    ...
    # 随机选取数组中的一个对象，替换关键字
    return random.choice(kfc).replace("木曜日", tb[idx] + "曜日").replace("Thursday", tb[idx + 1]).replace("THURSDAY", tb[
        idx + 1].upper()).replace("星期四", "星期" + tb[idx + 2]).replace("周四", "周" + tb[idx + 2])
